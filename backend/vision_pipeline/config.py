...
# Rover identifiers — driven by ROVER_COUNT env var (default 1 for single-robot testing)
def _rover_ids() -> tuple:
    count = int(os.getenv("ROVER_COUNT", "1"))
    return tuple(f"rover{i+1}" for i in range(max(1, count)))
